...
- `rule_urls: ["-https://..."]` / `["_https://..."]`: proxy-rule sources; matched domains use the proxy even if another rule source would direct them
- `rule_urls: ["!https://..."]`, `["！https://..."]`, or `["?https://..."]`: reject-rule sources; matched domains are blocked before any outbound connection is made
...
- `rule_urls` 里的 `-https://...` / `_https://...`：proxy 规则源，命中域名即使也命中直连规则也会走代理
- `rule_urls` 里的 `!https://...` / `！https://...` / `?https://...`：reject 规则源，命中域名时在发起任何外连前直接拒绝
